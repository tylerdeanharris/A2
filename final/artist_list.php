    	...
    	<div id="main-wrapper">
          ...
          <section id="main-content">
          	<article2>
               ...
               <h1 style="text-align:center";>Featured Artist</h1>
            <?php
				...
				if ($featuredResult['artist_id'] != $random) {
					...
					if(strlen($featuredResult['artist_description']) > 200) { 
						$str = substr($featuredResult['artist_description'], 0, 200); 
					}
					...
					echo '<p><b>Contact: </b>', $featuredResult['email'], '</p>';
					echo '</ul>';
					echo '</section>';
					echo '------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------';
				}
			...
			if($_SESSION['membership_type'] == 1 || $_SESSION['membership_type'] == 2) {
				echo '<a href=artist.php style="padding-left:20%";>Add New Artist</a>';
			}
					...
					foreach($result as $row) {
						echo '<section id="artist_list">';
							echo "<ul class='list_format'>";
								...
								echo '<p>', $str , '... <a href="artist_description.php?artist_id=', $row['artist_id'], '&artist_name=', $row['artist_name'], '">[Read More]</a></p>';
								echo '<p><b>Contact: </b>', $row['email'], '</p>';
							echo '</ul>';
						echo "</section>";
						if($_SESSION['id'] == $row['created_by'] || $_SESSION['membership_type'] == 2) {
							echo '<a href="update_form.php?id=' . $row['id'] . '">Edit</a> | <a href="delete_form.php?id=' . $row['id'] . '"  style="padding-left:20%";>Delete</a><br />';
						}
						echo "<br />";
					}
				}
				...
				foreach($dbh->query($sql3)as $row) {
					echo '<section id="artist_list">';
						echo "<ul class='list_format'>";
							...
							echo '<li><div><a href="artist_description.php?artist_id=', $row['artist_id'], '&artist_name=', $row['artist_name'], '"><h2>', $row['artist_name'], '</h2></a></div></li>';
							if(strlen($featuredResult['artist_description']) > 200) { 
								$str = substr($featuredResult['artist_description'], 0, 300); 
							}
							echo '<p>', $str , '... <a href="artist_description.php?artist_id=', $row['artist_id'], '&artist_name=', $row['artist_name'], '">[Read More]</a></p>';
							echo '<p><b>Contact: </b>', $row['email'], '</p>';
							echo '</ul>';
					echo "</section>";
					if($_SESSION['id'] == $row['created_by'] || $_SESSION['membership_type'] == 2) {
						echo '<a href="update_form.php?id=' . $row['id'] . '" style="padding-left:19%";>Edit</a> | <a href="delete_form.php?id=' . $row['id'] . '">Delete</a><br />';
					}
					echo "<br />";
				}
			}

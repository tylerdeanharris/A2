...
if ($_REQUEST['submit'] == "X"){
	$sql = "DELETE FROM information WHERE id = '$_REQUEST[id]'";
	if ($dbh->exec($sql))
...
if ($_REQUEST['submit'] == "Insert Entry"){
	$sql = "INSERT INTO information (name, description, price) VALUES ('$_REQUEST[name]', '$_REQUEST[description]', $_REQUEST[price])";
	echo "<p>Query: " . $sql . "</p>\n<p><strong>"; 
	if ($dbh->exec($sql))
...
{
	$sql = "UPDATE information SET name = '$_REQUEST[name]', phone = '$_REQUEST[phone]', address = '$_REQUEST[address]' WHERE id = '$_REQUEST[id]'";
	echo "<p>Query: " . $sql . "</p>\n<p><strong>"; 
	if ($dbh->exec($sql))
